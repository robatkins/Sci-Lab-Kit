...
slider1_min_value = -500
slider1_max_value = 500
...
slider2_min_value = -500
slider2_max_value = 500
...
while running:
    ...
    # Draw the electric field vectors as arrows
    for x in range(0, WIDTH, 40):
        for y in range(0, HEIGHT, 40):
            ...
            # Set color and direction based on magnitude and type (attractive/repulsive)
            magnitude = math.sqrt(electric_field[0]**2 + electric_field[1]**2)

            # Check the signs of the charges and reverse direction if both are positive
            if particle1_charge > 0 and particle2_charge > 0:
                scaled_field = [-component for component in scaled_field]

            color = (0, min(255, int(magnitude * 10)), 0)  # Green color for attractive, intensity based on magnitude
            if electric_field[0] * electric_field[1] < 0:
                color = (255, 0, 0)  # Red color for repulsive
            ...
            arrow_angle = math.atan2(electric_field[1], electric_field[0])
            arrowhead1 = (observation_point[0] + scaled_field[0] - arrow_length * math.cos(arrow_angle - math.pi / 6),
                          observation_point[1] + scaled_field[1] - arrow_length * math.sin(arrow_angle - math.pi / 6))
            arrowhead2 = (observation_point[0] + scaled_field[0] - arrow_length * math.cos(arrow_angle + math.pi / 6),
                          observation_point[1] + scaled_field[1] - arrow_length * math.sin(arrow_angle + math.pi / 6))

            # Draw arrows
            pygame.draw.line(screen, color, observation_point, 
                             (observation_point[0] + scaled_field[0], observation_point[1] + scaled_field[1]), 2)
            pygame.draw.polygon(screen, color, [observation_point, arrowhead1, arrowhead2])

    # Draw particles
    pygame.draw.circle(screen, (0, 0, 255), (int(particle1_pos[0]), int(particle1_pos[1])), 10)
